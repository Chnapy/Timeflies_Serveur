...
public class HCModele {
	...
	private static AnswerLogs getPlayerAnswerLogs(String pseudo) throws SQLException {
		...
		while (rsEntites.next()) {
			...
			if (InitialData.allEntites.get(rsEntites.getInt("idclasse")) instanceof ClasseEntitePassive) {
				ConsoleDisplay.error("La classe entité d'ID " + rsEntites.getInt("idclasse") + " est passive mais possède un joueur !");
			}
			ClasseEntiteActive cea = (ClasseEntiteActive) InitialData.allEntites.get(rsEntites.getInt("idclasse"));
	...
	public static HCPersonnage getHCPersonnageFromClasseEntite(ClasseEntiteActive cea, int identite,
		...
		for (int i = 0; i < hcsa.length; i++) {
			...
			hcsa[i] = new HCSortActif(sa.nom, sortsNiveaux.get(sa.id),
		...
		for (int i = 0; i < hcsp.length; i++) {
			...
			hcsp[i] = new HCSortPassif(sp.nom, sortsNiveaux.get(sp.id),
					sp.description);
		}

		return new HCPersonnage(identite, nomDonne, cea.nomClasse, persoNiveau,
